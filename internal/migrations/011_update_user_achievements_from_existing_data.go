...
func UpdateUserAchievementsFromExistingData(db *gorm.DB) error {
	return db.Transaction(
		func(tx *gorm.DB) error {
			...
			`, userLogin,
				).Scan(&subscribersCount).Error; err != nil {
			...
			`, userLogin,
				).Scan(&collectionsLikes).Error; err != nil {
			...
			`, userLogin,
				).Scan(&photosLikes).Error; err != nil {
			...
			`, userLogin,
				).Scan(&reactionCount).Error; err != nil {
				...
				`, exp, level, currentValue, currentBool, userLogin, code,
					).Error
				}
				...
				if stats.HasCollections {
					if err := updateAchievement(
						AchieveFirstCollectionCreate,
						XPFirstCollectionCreate,
						1,
						0,
						true,
					); err != nil {
				...
				if stats.YearsRegistered >= 1 {
					...
					if err := updateAchievement(
						AchieveYearlyRegister,
						yearlyExp,
						1,
						stats.YearsRegistered,
						false,
					); err != nil {
						log.Printf("Ошибка обновления yearlyRegister для %s: %v", userLogin, err)
					} else {
						log.Printf(
							"Начислено годы регистрации для %s: %d лет, %d XP",
							userLogin,
							stats.YearsRegistered,
							yearlyExp,
						)
				...
				if stats.CollectionItemsCount > 0 {
					...
					if itemsExp > 0 {
						if err := updateAchievement(
							AchieveCollectionItemsAdded,
							itemsExp,
							itemsLevel,
							stats.CollectionItemsCount,
							false,
						); err != nil {
							log.Printf("Ошибка обновления collectionItemsAdded для %s: %v", userLogin, err)
						} else {
							log.Printf(
								"Начислено экземпляры для %s: %d XP (уровень %d)",
								userLogin,
								itemsExp,
								itemsLevel,
							)
				...
				if stats.CollectionsLikes > 0 {
					...
					if likesExp > 0 {
						if err := updateAchievement(
							AchieveCollectionsLikes,
							likesExp,
							likesLevel,
							stats.CollectionsLikes,
							false,
						); err != nil {
							log.Printf("Ошибка обновления collectionsLikes для %s: %v", userLogin, err)
						} else {
							log.Printf(
								"Начислено лайки на коллекциях для %s: %d XP (уровень %d)",
								userLogin,
								likesExp,
								likesLevel,
							)
				...
				if stats.CollectionItemsLikes > 0 {
					...
					if likesExp > 0 {
						if err := updateAchievement(
							AchieveCollectionItemsLikes,
							likesExp,
							likesLevel,
							stats.CollectionItemsLikes,
							false,
						); err != nil {
							log.Printf("Ошибка обновления collectionItemsLikes для %s: %v", userLogin, err)
						} else {
							log.Printf(
								"Начислено лайки на экземплярах для %s: %d XP (уровень %d)",
								userLogin,
								likesExp,
								likesLevel,
							)
				...
				if stats.PhotosLikes > 0 {
					...
					if likesExp > 0 {
						if err := updateAchievement(
							...
							log.Printf(
								"Начислено лайки на фото для %s: %d XP (уровень %d)",
								userLogin,
								likesExp,
								likesLevel,
							)
				...
				if stats.PostsReactions > 0 {
					...
					if reactionsExp > 0 {
						if err := updateAchievement(
							AchievePostsReactions,
							reactionsExp,
							reactionsLevel,
							stats.PostsReactions,
							false,
						); err != nil {
							log.Printf("Ошибка обновления postsReactions для %s: %v", userLogin, err)
						} else {
							log.Printf(
								"Начислено реакции на посты для %s: %d XP (уровень %d)",
								userLogin,
								reactionsExp,
								reactionsLevel,
							)
				...
				if stats.CollectionsSum > 0 {
					...
					if sumExp > 0 {
						if err := updateAchievement(
							AchieveCollectionsSum,
							sumExp,
							sumLevel,
							stats.CollectionsSum,
							false,
						); err != nil {
							log.Printf("Ошибка обновления collectionsSum для %s: %v", userLogin, err)
						} else {
							log.Printf(
								"Начислено сумма на коллекции для %s: %d XP (уровень %d)",
								userLogin,
								sumExp,
								sumLevel,
							)
				...
				if stats.CollectionShipSum > 0 {
					...
					if shipExp > 0 {
						if err := updateAchievement(
							AchieveCollectionShipSum,
							shipExp,
							shipLevel,
							stats.CollectionShipSum,
							false,
						); err != nil {
							log.Printf("Ошибка обновления collectionShipSum для %s: %v", userLogin, err)
						} else {
							log.Printf(
								"Начислено сумма на доставки для %s: %d XP (уровень %d)",
								userLogin,
								shipExp,
								shipLevel,
							)
